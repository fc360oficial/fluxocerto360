...
(function (root, factory) {
  ...
  function criarCatalogo(itens) {
    ...
    (itens || []).forEach(function (it) {
      ...
      if (!c && !e) return;
      total++;
      var item = { c: c, e: e, d: it.d || '', u: it.u || '', q: (it.q == null ? null : Number(it.q)), k: (it.k == null ? null : Number(it.k)), v: (it.v == null ? null : Number(it.v)) };
  ...
  function _fmt(it) { return { codigo: it.c, ean: it.e, desc: it.d, un: it.u, estoque: it.q, custo: (it.k == null ? null : it.k), venda: (it.v == null ? null : it.v) }; }
  ...
  function montarBlocos(itens, tam) {
    ...
    for (var i = 0; i < itens.length; i += tam) {
      out.push(itens.slice(i, i + tam).map(function (it) {
        return { c: it.c || '', e: it.e || '', d: String(it.d || '').slice(0, 120), u: it.u || '', q: (it.q == null ? null : it.q), k: (it.k == null ? null : it.k), v: (it.v == null ? null : it.v) };
      }));
    }
    return out;
  }
  ...
  function mapearColunas(header, linhas) {
    ...
    var ean = idx(/ean|gtin|barra/), codigo = idx(/^cod|codigo|código|sku/), desc = idx(/desc|nome|produto/), un = idx(/^un|unid|emb/), estoque = idx(/estoq|saldo|qtd/), custo = idx(/custo/), venda = idx(/pre[çc]o|venda/);
    ...
    if (!temHeader) {
      ...
      ean = codigo = desc = un = estoque = custo = venda = -1;
    ...
    return { codigo: codigo, ean: ean, desc: desc, un: un, estoque: estoque, custo: custo, venda: venda, temHeader: temHeader };
  }
  ...
  function calcularResultado(cat, bips, resolucoes) {
    ...
    var contado = {}, porEnd = {}, nBips = {}, endsPor = {};
    function keyOf(b) {
    ...
    (bips || []).forEach(function (b) {
      ...
      contado[k] = (contado[k] || 0) + q; nBips[k] = (nBips[k] || 0) + 1;
      if (b.endereco) { endsPor[k] = endsPor[k] || {}; endsPor[k][b.endereco] = 1; }
      // Por endereço: bipagens normais e correções gravadas num endereço real ('_CORRECAO' fica de fora).
      if (b.endereco && b.endereco !== '_CORRECAO') { porEnd[b.endereco] = porEnd[b.endereco] || {}; porEnd[b.endereco][k] = (porEnd[b.endereco][k] || 0) + q; }
    });
    function mk(k, it, cont, nc) {
      var sis = (it.q == null ? null : Number(it.q));
      var dif = (sis == null ? null : cont - sis);
      var custo = (it.k == null ? null : Number(it.k)), venda = (it.v == null ? null : Number(it.v));
      return { key: k, venda: venda, vendaContada: (venda != null) ? cont * venda : null, codigo: it.c || '', ean: it.e || '', desc: it.d || '', un: it.u || '', sistema: sis, contado: cont, dif: dif, custo: custo, valor: (dif != null && custo != null) ? dif * custo : null, valorSistema: (sis != null && custo != null) ? sis * custo : null, valorContado: (custo != null) ? cont * custo : null, nc: !!nc, bips: nBips[k] || 0, ends: endsPor[k] ? Object.keys(endsPor[k]).length : 0 };
    }
    var linhas = [], vistos = {};
    ...
    Object.keys(contado).forEach(function (k) { if (vistos[k]) return; vistos[k] = 1; linhas.push(mk(k, { c: k.indexOf('c:') === 0 ? k.slice(2) : '', e: k.indexOf('e:') === 0 ? k.slice(2) : '', d: '(não cadastrado)', u: '', q: null, k: null, v: null }, contado[k], true)); });
    var tot = { itens: linhas.length, divergentes: 0, sobraUn: 0, faltaUn: 0, sobraVal: 0, faltaVal: 0, semCusto: 0, naoCadastrados: 0, coletados: 0, naoColetados: 0, naoColetadosUn: 0, naoColetadosVal: 0, contadoUn: 0, contadoVal: 0, sistemaUn: 0, sistemaVal: 0 };
    linhas.forEach(function (l) {
      if (l.nc) tot.naoCadastrados++;
      if (l.contado) tot.coletados++;
      tot.contadoUn += l.contado || 0; if (l.valorContado != null) tot.contadoVal += l.valorContado;
      if (l.sistema != null) { tot.sistemaUn += l.sistema; if (l.valorSistema != null) tot.sistemaVal += l.valorSistema; }
      // Produto sem nenhuma bipagem: não entra em sobra/falta (ainda não foi contado), fica num total próprio
      if (!l.contado && l.sistema != null && l.sistema !== 0) { tot.naoColetados++; tot.naoColetadosUn += Math.abs(l.sistema); if (l.valorSistema != null) tot.naoColetadosVal += Math.abs(l.valorSistema); return; }
    ...
    return { linhas: linhas, totais: tot, enderecos: enderecos, porEnd: porEnd };
  }
