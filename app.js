  ...
  var html =
    ...
    '<div style="background:#fff;border-radius:20px;width:100%;max-width:400px;overflow:hidden;box-shadow:0 20px 60px rgba(0,0,0,.3)">'+
      ...
      '<div style="background:linear-gradient(135deg,#1a1a1a,#2d2d2d);padding:24px 24px 20px;color:#fff;text-align:center">'+
        ...
        '<div style="font-family:\'Plus Jakarta Sans\',sans-serif;font-size:18px;font-weight:800;margin-bottom:2px">'+nomeCliente+'</div>'+
      ...
      '<div style="padding:20px 24px">'+
        ...
        '<button onclick="ativarToken(true)" style="width:100%;padding:13px;background:var(--y);border:none;border-radius:12px;font-size:14px;font-weight:800;font-family:\'Plus Jakarta Sans\',sans-serif;cursor:pointer">Ativar Token</button>'+
  ...
  if (!ov) {
    ...
    ov.innerHTML =
      '<div style="background:#fff;border-radius:18px;padding:36px 28px;max-width:380px;text-align:center;width:100%">'+
        ...
        '<div style="font-family:\'Plus Jakarta Sans\',sans-serif;font-size:20px;font-weight:800;color:#c0392b;margin-bottom:10px">Acesso Bloqueado</div>'+
        ...
        '<button onclick="ativarToken()" style="width:100%;padding:12px;background:var(--y);border:none;border-radius:10px;font-size:14px;font-weight:800;font-family:\'Plus Jakarta Sans\',sans-serif;cursor:pointer">Ativar Token</button>'+
...
function setupRole() {
  ...
  show('sb-promotores-sec', (isAdmin || isSup) && _moduloAtivo('promotores'));
...
function buildCLBlock(cl) {
  ...
  return '<div style="background:#fff;border:1px solid var(--gray2);border-radius:14px;padding:20px;margin-bottom:16px;box-shadow:var(--sh)">'
    ...
    + '<div>'
    + '<div style="font-family:\'Plus Jakarta Sans\',sans-serif;font-size:18px;font-weight:800;color:var(--t);margin-bottom:4px">' + clLabel + '</div>'
...
function verDetalhe(id) {
  ...
  document.getElementById('det-kpis').innerHTML = [
    ...
    return '<div style="background:'+k.bg+';border-radius:12px;padding:16px 14px;text-align:center">'
      +'<div style="font-size:24px;font-weight:800;color:'+k.color+';font-family:\'Plus Jakarta Sans\',sans-serif">'+k.val+'</div>'
...
function renderPromotoresPainel() {
  ...
  Promise.all([
    ...
    // Celular = operação do dia (gerente na loja); desktop = retaguarda completa.
    var mobile = _promIsMobile();
    document.getElementById('promotores-desktop').style.display = mobile ? 'none' : '';
    document.getElementById('promotores-mobile').style.display = mobile ? '' : 'none';
    if (mobile) { renderPromotoresMobile(); return; }
...
// ── Promotores: modo celular (operação do dia) ──
// Quem usa: gerente/supervisor em pé na loja. Só o que precisa na hora:
// quem está na loja, quem ainda vai chegar hoje, mostrar o QR na tela pro
// promotor ler (loja sem cartaz) e registrar entrada manual (promotor sem
// celular). Cadastros, agendamento recorrente, rankings e impressão ficam
// só no desktop (#promotores-desktop).
function _promIsMobile() { return window.innerWidth <= 768; }

// Loja "atual" do aparelho: última escolhida neste aparelho > loja do
// cadastro do usuário > primeira loja com fornecedor. Só lojas que existem
// em algum fornecedor contam (mesma fonte da grade de QR).
function _promLojaAtual() {
  var lojas = getLojasUnicas(S_PROM.fornecedores);
  var salva = ''; try { salva = localStorage.getItem('fc360_prom_loja') || ''; } catch (e) {}
  var userLoja = (S.currentUser && S.currentUser.loja || '').trim();
  var cand = [salva, userLoja].filter(Boolean);
  for (var i = 0; i < cand.length; i++) {
    var m = lojas.filter(function(l) { return String(l).toLowerCase() === cand[i].toLowerCase(); })[0];
    if (m) return m;
  }
  return lojas[0] || '';
}

function trocarLojaPromotoresMobile(loja) {
  try { localStorage.setItem('fc360_prom_loja', loja); } catch (e) {}
  renderPromotoresMobile();
}

function _promTs(ts) { if (!ts) return 0; return ts.toDate ? ts.toDate().getTime() : new Date(ts).getTime(); }

function _promHora(ts) {
  if (!ts) return '-';
  return new Date(_promTs(ts)).toLocaleTimeString('pt-BR', {hour: '2-digit', minute: '2-digit'});
}

function _promDecorrido(ts) {
  if (!ts) return '';
  var min = Math.max(0, Math.round((Date.now() - _promTs(ts)) / 60000));
  return min < 60 ? 'há ' + min + ' min' : 'há ' + Math.floor(min / 60) + 'h' + String(min % 60).padStart(2, '0');
}

function _promMesmaLoja(v, loja) {
  return String(v.lojaId || v.lojaNome || '').toLowerCase() === String(loja).toLowerCase();
}

function renderPromotoresMobile() {
  var hoje = getLocalDate();
  var lojas = getLojasUnicas(S_PROM.fornecedores);
  var loja = _promLojaAtual();
  var sel = document.getElementById('pm-loja');
  sel.innerHTML = lojas.map(function(l) {
    return '<option value="' + _escHtml(l) + '"' + (l === loja ? ' selected' : '') + '>Loja ' + _escHtml(l) + '</option>';
  }).join('');
  sel.style.display = lojas.length > 1 ? '' : 'none';
  ...
  document.getElementById('pm-data').textContent =
    dataTxt.charAt(0).toUpperCase() + dataTxt.slice(1)
    + (lojas.length <= 1 && loja ? ' · Loja ' + loja : '');

  var daLoja = S_PROM.visitas.filter(function(v) { return _promMesmaLoja(v, loja); });
  var naLoja = daLoja.filter(function(v) { return v.status === 'na_loja'; })
    .sort(function(a, b) { return _promTs(a.checkInEm) - _promTs(b.checkInEm); });
  var agenda = daLoja.filter(function(v) { return v.status === 'agendada' && v.dataAgendada === hoje; })
    .sort(function(a, b) { return (a.horaAgendada || '99') < (b.horaAgendada || '99') ? -1 : 1; });
  var encerradas = daLoja.filter(function(v) { return v.dataAgendada === hoje && (v.status === 'realizada' || v.status === 'nao_compareceu'); })
    .sort(function(a, b) { return _promTs(b.checkOutEm) - _promTs(a.checkOutEm); });

  var vazio = function(msg) { return '<div class="empty" style="padding:14px">' + msg + '</div>'; };

  document.getElementById('pm-na-loja').innerHTML = !lojas.length
    ? vazio('Nenhum fornecedor cadastrado. Cadastre pelo computador.')
    : (naLoja.length ? naLoja.map(function(v) {
      return '<div class="pm-card"><div><b>' + _escHtml(v.fornecedorNome || '-') + '</b>'
        + '<div class="pm-sub">' + _escHtml(v.promotorNome || '-') + ' · entrou às ' + _promHora(v.checkInEm) + ' · ' + _promDecorrido(v.checkInEm) + '</div></div>'
        + '<div class="pm-acts"><button class="btn btn-s btn-sm" onclick="fecharVisitaManualmente(\'' + v.id + '\')">Encerrar</button></div></div>';
    }).join('') : vazio('Ninguém na loja agora.'));

  document.getElementById('pm-agenda').innerHTML = agenda.length ? agenda.map(function(v) {
    return '<div class="pm-card"><div><b>' + _escHtml(v.fornecedorNome || '-') + '</b>'
      + '<div class="pm-sub">' + (v.horaAgendada ? v.horaAgendada + ' · ' : '') + _escHtml(v.promotorNome || 'promotor não informado') + '</div></div>'
      + '<div class="pm-acts"><button class="btn btn-p btn-sm" onclick="marcarChegadaManual(\'' + v.id + '\')">Chegou</button>'
      + '<button class="btn btn-s btn-sm" onclick="marcarNaoCompareceu(\'' + v.id + '\')">Não veio</button></div></div>';
  }).join('') : vazio('Nenhuma visita agendada pra hoje.');

  document.getElementById('pm-encerradas').innerHTML = encerradas.length ? encerradas.map(function(v) {
    var st = STATUS_VISITA[v.status] || {label: v.status, cls: 'st-info'};
    var det = v.status === 'realizada' ? _promHora(v.checkInEm) + ' → ' + _promHora(v.checkOutEm) : (v.horaAgendada || '');
    return '<div class="pm-card"><div><b>' + _escHtml(v.fornecedorNome || '-') + '</b>'
      + '<div class="pm-sub">' + _escHtml(v.promotorNome || '-') + (det ? ' · ' + det : '') + '</div></div>'
      + '<span class="st ' + st.cls + '">' + st.label + '</span></div>';
  }).join('') : vazio('Nada encerrado ainda hoje.');
}

// Promotor agendado chegou mas não leu o QR — gerente registra a entrada.
// Usa a regra Update #3 (admin/supervisor edita qualquer visita).
function marcarChegadaManual(id) {
  visitasCol().doc(id).update({
    status: 'na_loja',
    checkInEm: firebase.firestore.FieldValue.serverTimestamp(),
    registradoPor: (S.currentUser && S.currentUser.nome) || null
  }).then(function() { showToast('Entrada registrada.'); renderPromotoresPainel(); })
    .catch(function(e) { showToast('Erro: ' + e.message); });
}

function marcarNaoCompareceu(id) {
  if (!confirm('Marcar como "não compareceu"?')) return;
  visitasCol().doc(id).update({ status: 'nao_compareceu' })
    .then(function() { showToast('Marcado como não compareceu.'); renderPromotoresPainel(); })
    .catch(function(e) { showToast('Erro: ' + e.message); });
}

// QR da loja em tela cheia, pro promotor ler direto do celular do gerente
// (loja sem cartaz, cartaz rasgado).
function abrirQrTelaCheia() {
  var loja = _promLojaAtual();
  if (!loja) { showToast('Nenhuma loja com fornecedor cadastrado.'); return; }
  var url = location.origin + location.pathname + '?checkin=1&c=' + S.clienteConfig.id + '&l=' + loja;
  var qr = qrcode(0, 'M'); qr.addData(url); qr.make();
  document.getElementById('pqf-cliente').textContent = (S.clienteConfig && S.clienteConfig.nome) || '';
  document.getElementById('pqf-loja').textContent = 'Loja ' + loja;
  var box = document.getElementById('pqf-qr');
  box.innerHTML = qr.createSvgTag(6);
  var svg = box.querySelector('svg');
  var size = Math.min(window.innerWidth - 88, window.innerHeight - 260, 360);
  if (svg) { svg.setAttribute('width', size); svg.setAttribute('height', size); }
  document.getElementById('prom-qr-full').style.display = 'flex';
}

function fecharQrTelaCheia() { document.getElementById('prom-qr-full').style.display = 'none'; }

// Registro manual de entrada (promotor sem celular / câmera quebrada).
function abrirModalVisitaManual() {
  var loja = _promLojaAtual();
  if (!loja) { showToast('Nenhuma loja com fornecedor cadastrado.'); return; }
  var forns = S_PROM.fornecedores.filter(function(f) {
    return f.ativo !== false && (f.lojas || []).some(function(l) { return String(l).toLowerCase() === loja.toLowerCase(); });
  });
  document.getElementById('vm-fornecedor').innerHTML = '<option value="">Selecione...</option>'
    + forns.map(function(f) { return '<option value="' + _escHtml(f.id) + '">' + _escHtml(f.nome) + '</option>'; }).join('');
  document.getElementById('vm-nome').value = '';
  document.getElementById('vm-loja').value = 'Loja ' + loja;
  document.getElementById('modal-visita-manual').style.display = 'flex';
}

function salvarVisitaManual() {
  var fid = document.getElementById('vm-fornecedor').value;
  var nome = document.getElementById('vm-nome').value.trim();
  if (!fid) { showToast('Selecione o fornecedor.'); return; }
  if (!nome) { showToast('Informe o nome do promotor.'); return; }
  var f = S_PROM.fornecedores.filter(function(x) { return x.id === fid; })[0];
  var loja = _promLojaAtual();
  // Regra do Firestore: usuário logado só CRIA visita como 'agendada'
  // (Create #1). A entrada é um segundo passo via Update #3 — por isso
  // dois writes em vez de um add() já com status 'na_loja'.
  var base = {
    fornecedorId: f.id, fornecedorNome: f.nome,
    lojaId: loja, lojaNome: loja,
    promotorNome: nome, promotorTelefone: null,
    dataAgendada: getLocalDate(), horaAgendada: null,
    status: 'agendada', sessionUid: null,
    checkInEm: null, checkOutEm: null, checkInGeo: null, checkOutGeo: null,
    manual: true, registradoPor: (S.currentUser && S.currentUser.nome) || null
  };
  visitasCol().add(base).then(function(ref) {
    return ref.update({ status: 'na_loja', checkInEm: firebase.firestore.FieldValue.serverTimestamp() });
  }).then(function() {
    document.getElementById('modal-visita-manual').style.display = 'none';
    showToast('Entrada registrada.');
    renderPromotoresPainel();
  }).catch(function(e) { showToast('Erro ao registrar: ' + e.message); });
}
...
function _renderResultadoBalanco() {
  ...
  // Filtro por endereço: mostra só os produtos contados nele. Qtd Contado é a do endereço; Sistema, Divergência e R$ são do produto na loja inteira (o sistema não tem estoque por endereço).
  ...
  var difUn=(t.contadoUn||0)-(t.sistemaUn||0), difVal=(t.contadoVal||0)-(t.sistemaVal||0);
  ...
  wrap.innerHTML=
    (endSel?'<div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:12px">'+tilesEnd+'</div>':'<div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:12px">'+
      tile('Estoque no sistema',(+(t.sistemaUn||0).toFixed(2)).toLocaleString('pt-BR')+' un'+val(t.sistemaVal||0)+sub('quantidade do sistema × custo'),'var(--t2)')+
      tile('Contado',(+(t.contadoUn||0).toFixed(2)).toLocaleString('pt-BR')+' un'+val(t.contadoVal||0)+sub('quantidade contada × custo'),'#1a5c34')+
      tile('Diferença',(difUn>0?'+':'')+(+difUn.toFixed(2)).toLocaleString('pt-BR')+' un'+val(difVal)+sub('contado − sistema'),difVal<0?'#c0392b':'#1a5c34')+
    '</div>')+
...
function _exportarResultadoCsv() {
  ...
  var fonte=mapa?_resultadoCache.r.linhas.filter(function(l){ return mapa[l.key]!=null; }).map(function(l){ var q=mapa[l.key]; return Object.assign({}, l, {contado:q, contadoTotal:l.contado, valorContado:(l.custo!=null?q*l.custo:null)}); }):_resultadoCache.r.linhas;
  if (mapa) lines[0]='ENDERECO;'+lines[0].replace('QTD_CONTADO;','QTD_CONTADO_ENDERECO;QTD_CONTADO_TOTAL;');
  fonte.forEach(function(l){ lines.push([(mapa?endSel+';':'')+l.codigo,l.ean,l.desc.replace(/;/g,' '),l.un,f(l.sistema),f(l.contado)+(mapa?';'+f(l.contadoTotal):''),f(l.dif),f(l.custo),f(l.valorSistema==null?null:+l.valorSistema.toFixed(2)),f(l.valorContado==null?null:+l.valorContado.toFixed(2)),f(l.valor==null?null:+l.valor.toFixed(2)),l.nc?'SIM':'',l.bips||0,l.ends||0].join(';')); });
...
function _etcAtualizarStatusUI() {
  if (_etcCurrentView === 'hub') renderEtcHub();
    ...
    // redesenhar do zero navegaria o operador pra fora da tela de revisão.
    // Atualiza só o pill e o disabled/title do botão de imprimir, ao vivo.
...
function renderEtcRevisaoLote() {
  // _etcMontandoLote precisa ser zerado aqui — sem isso ele fica true (deixado
  // por renderEtcMontarLote) enquanto o operador está na Revisão, e o branch
  // "else if (_etcMontandoLote)" de _etcAtualizarStatusUI (que vem antes do
  // branch "else if (_etcRevisandoLote)" na cadeia) intercepta o disconnect
  // silenciosamente, reproduzindo o bug original (ver Finding 3 da revisão final).
  _etcMontandoLote = false;
  _etcFilaInterrompidaAtiva = false;
...
// true enquanto a tela de Revisão do lote (renderEtcRevisaoLote) está
// aberta — permite a _etcAtualizarStatusUI atualizar o pill/botão de
  ...
  if (opsHojeWrap) {
    if (!resFilt.length) {
      ...
      resFilt.forEach(function(r){
        if (!opMap[r.operador]) opMap[r.operador] = {nome:r.operador, loja:r.loja||'', perfil:r.perfil||'', envios:0, totalPct:0};
      ...
      var opGrads={gerencia:'linear-gradient(135deg,#3b82f6,#1d4ed8)',supervisor:'linear-gradient(135deg,#8b5cf6,#6d28d9)',prevencao:'linear-gradient(135deg,#f59e0b,#b45309)',operator:'linear-gradient(135deg,#10b981,#047857)',admin:'linear-gradient(135deg,#ef4444,#b91c1c)'};
      opsHojeWrap.innerHTML = opList.map(function(op,i){
        ...
        var avatarBg = opGrads[op.perfil]||'linear-gradient(135deg,#6b7280,#4b5563)';
        return '<div style="display:flex;align-items:center;gap:10px;padding:8px 10px;border-radius:10px;background:var(--w);border:1px solid rgba(0,0,0,.07)">'
          +'<div style="width:34px;height:34px;border-radius:50%;background:'+avatarBg+';display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700;color:#fff;flex-shrink:0">'+ini+'</div>'
          +'<div style="flex:1;min-width:0">'
...
function _mostrarFaixaImpersonar(nomeCliente) {
  ...
  faixa.style.cssText = 'position:fixed;top:0;left:0;right:0;width:100%;box-sizing:border-box;z-index:10000;background:#1a1a1a;color:#fff;padding:8px 12px;font-size:12.5px;font-weight:600;font-family:\'Plus Jakarta Sans\',sans-serif;box-shadow:0 2px 8px rgba(0,0,0,.2);text-align:center';
  faixa.innerHTML = '<div style="display:block;width:100%;box-sizing:border-box;word-break:break-word;overflow-wrap:break-word;margin-bottom:6px">🔧 Operando como <strong>'+nomeCliente+'</strong> (via Superadmin)</div>'
  ...
  var cards = clientesFiltrados.map(function(c) {
    ...
    return '<div class="card" style="margin-bottom:12px;padding:0;overflow:hidden;border:1.5px solid var(--gray2)'+(inativo?';opacity:.6':'')+'">' +
      ...
      '<div style="padding:14px 18px;display:flex;align-items:center;justify-content:space-between;gap:12px;border-bottom:1px solid var(--gray2)">' +
        '<div>' +
          '<div style="font-size:16px;font-weight:700;letter-spacing:.2px;margin-bottom:2px;font-family:\'Plus Jakarta Sans\',sans-serif">'+c.nome+'</div>' +
  ...
  var baseCard =
    '<div style="background:linear-gradient(135deg,#1a1a1a,#2d2d2d);border-radius:14px;padding:18px 22px;margin-bottom:20px;color:#fff;display:flex;align-items:center;justify-content:space-between;gap:16px;flex-wrap:wrap">' +
      '<div>' +
        ...
        '<div style="font-size:20px;font-weight:700;letter-spacing:.3px;font-family:\'Plus Jakarta Sans\',sans-serif">Fluxo Certo 360</div>' +
      ...
      '<div style="display:flex;align-items:center;gap:18px;flex-wrap:wrap">' +
        ...
        '</div>' +
        '<button onclick="deployTodosClientes()" style="padding:10px 20px;background:#f1c40f;color:#1a1a1a;border:none;border-radius:10px;font-size:13px;font-weight:700;font-family:\'Plus Jakarta Sans\',sans-serif;cursor:pointer;white-space:nowrap">🚀 Publicar para Todos</button>' +
      '</div>' +
    '</div>';
...
function abrirEditarCliente(clienteId) {
  ...
  var html = '<div id="modal-editar-cliente" onclick="if(event.target===this)fecharEditarCliente()" style="position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:3000;display:flex;align-items:center;justify-content:center;padding:16px">'+
    '<div style="background:#fff;border-radius:16px;padding:28px 24px;width:100%;max-width:440px;max-height:90vh;overflow-y:auto">'+
      '<div style="font-family:\'Plus Jakarta Sans\',sans-serif;font-size:17px;font-weight:800;margin-bottom:18px">Editar Cliente</div>'+
...
function abrirNovoCliente() {
  ...
  var html = '<div id="modal-novo-cliente" onclick="if(event.target===this)fecharNovoCliente()" style="position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:3000;display:flex;align-items:center;justify-content:center;padding:16px">'+
    '<div style="background:#fff;border-radius:16px;padding:28px 24px;width:100%;max-width:440px;max-height:90vh;overflow-y:auto">'+
      '<div style="font-family:\'Plus Jakarta Sans\',sans-serif;font-size:17px;font-weight:800;margin-bottom:18px">Novo Cliente</div>'+
...
function gerarTokenCliente(clienteId) {
  ...
  var html =
    ...
    '<div style="background:#fff;border-radius:16px;padding:28px 24px;width:100%;max-width:400px">'+
      '<div style="font-family:\'Plus Jakarta Sans\',sans-serif;font-size:17px;font-weight:800;margin-bottom:4px">Gerar Token de Licença</div>'+
  ...
  db.collection('tokens').add({
    ...
    var html2 =
      ...
      '<div style="background:#fff;border-radius:16px;padding:28px 24px;width:100%;max-width:400px;text-align:center">'+
        ...
        '<div style="font-family:\'Plus Jakarta Sans\',sans-serif;font-size:17px;font-weight:800;margin-bottom:16px">Token Gerado!</div>'+
        ...
        '</div>'+
        '<button onclick="var t=document.getElementById(\'token-display\').textContent;navigator.clipboard.writeText(t).then(function(){showToast(\'✅ Token copiado!\');}).catch(function(){});" style="width:100%;padding:12px;background:var(--y);border:none;border-radius:10px;font-size:14px;font-weight:800;font-family:\'Plus Jakarta Sans\',sans-serif;cursor:pointer;margin-bottom:8px">📋 Copiar Token</button>'+
  ...
  db.collection('tokens').where('clienteId','==',clienteId).limit(20).get().then(function(snap) {
    ...
    var html = '<div id="modal-tokens" onclick="if(event.target===this)this.remove()" style="position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:3000;display:flex;align-items:center;justify-content:center;padding:16px">'+
      '<div style="background:#fff;border-radius:16px;padding:24px;width:100%;max-width:440px;max-height:80vh;overflow-y:auto">'+
        '<div style="font-family:\'Plus Jakarta Sans\',sans-serif;font-size:16px;font-weight:800;margin-bottom:16px">Tokens — '+clienteId+'</div>'+
        rows+
...
function enviarWhatsApp() {
  ...
  var html =
    ...
    '<div style="background:#fff;border-radius:18px;padding:24px 22px;width:100%;max-width:400px;box-shadow:0 8px 40px rgba(0,0,0,.25)">'+
      '<div style="font-family:\'Plus Jakarta Sans\',sans-serif;font-size:17px;font-weight:800;margin-bottom:4px">📱 Avisar pelo WhatsApp</div>'+
    ...
    var html=
      '<div id="modal-encerrar-inv" onclick="if(event.target===this)fecharModalEncerrarInv()" '+
        ...
        '<div style="background:#fff;border-radius:16px;padding:28px 24px 24px;width:100%;max-width:440px;box-shadow:0 8px 40px rgba(0,0,0,.22)">'+
          '<div style="font-family:\'Plus Jakarta Sans\',sans-serif;font-size:18px;font-weight:800;margin-bottom:6px;color:#c0392b">⚠ Encerrar Inventário</div>'+
...
function renderCorrecaoBipagem() {
  ...
  wrap.innerHTML=
    '<div class="card" style="max-width:480px">'+
      '<div style="font-family:\'Plus Jakarta Sans\',sans-serif;font-size:15px;font-weight:800;margin-bottom:4px">Correção de Bipagem</div>'+
...
function _abrirModalExportErp(perfil) {
  ...
  var html =
    ...
    '<div style="background:#fff;border-radius:18px;padding:28px 24px;width:100%;max-width:520px;max-height:90vh;overflow-y:auto;box-shadow:0 8px 40px rgba(0,0,0,.25)">'+
      '<div style="font-family:\'Plus Jakarta Sans\',sans-serif;font-size:18px;font-weight:800;margin-bottom:4px">Exportar para ERP</div>'+
...
function renderInvComparativo() {
  ...
  el.innerHTML='<div class="card" style="margin-bottom:14px">'+
    '<div style="font-family:\'Plus Jakarta Sans\',sans-serif;font-size:15px;font-weight:700;margin-bottom:12px">Comparativo por Endereço + EAN</div>'+
...
function _renderSelecaoEndereco(inv, bipCount) {
  ...
  if (inv.modoOrganizacao === 'setores') {
    ...
    var btns=ends.map(function(s,i){
      ...
      var ss=s.replace(/'/g,"\\'");
      return '<button onclick="_confirmarSetorFila(\''+safeInvId+'\',\''+ss+'\',\''+ss+'\')" style="padding:20px 6px;'+border+';border-radius:14px;'+bg+';'+cl+';font-size:14px;font-weight:800;font-family:\'Plus Jakarta Sans\',sans-serif;cursor:pointer;width:100%;box-sizing:border-box;word-break:break-word;line-height:1.4">'+icon+'<br>'+s+(andamento?'<br><span style="font-size:10px;font-weight:600">em andamento</span>':'')+'</button>';
    }).join('');
    return '<div>'+
      '<div style="font-family:\'Plus Jakarta Sans\',sans-serif;font-size:16px;font-weight:700;margin-bottom:6px">Selecionar Setor</div>'+
  ...
  return '<div>'+
    '<div style="font-family:\'Plus Jakarta Sans\',sans-serif;font-size:16px;font-weight:700;margin-bottom:4px">Selecionar Endereço</div>'+
  ...
  var html=
    '<div id="modal-setor-picker" onclick="if(event.target===this)_fecharSetorPicker()" style="position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:2100;display:flex;align-items:flex-end;justify-content:center;padding:0">'+
      '<div style="background:#fff;border-radius:20px 20px 0 0;padding:24px 20px 36px;width:100%;max-width:480px;box-shadow:0 -4px 32px rgba(0,0,0,.18)">'+
        '<div style="font-family:\'Plus Jakarta Sans\',sans-serif;font-size:17px;font-weight:800;margin-bottom:4px">Selecionar Setor</div>'+
...
function _htmlIdColetorForm() {
  ...
  return '<div style="max-width:360px;margin:50px auto;padding:28px 24px;background:#fff;border-radius:16px;border:1px solid var(--gray2);box-shadow:var(--sh)">'+
    '<div style="font-family:\'Plus Jakarta Sans\',sans-serif;font-size:19px;font-weight:800;margin-bottom:6px">Identificação</div>'+
...
function renderColeta() {
  ...
  wrap.innerHTML=coletorChip+mudarBtn+
    '<div style="background:#fff;border-radius:14px;border:1px solid var(--gray2);padding:20px;box-shadow:var(--sh);margin-bottom:16px">'+
      '<div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px">'+
        '<div><div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.8px;color:var(--t3)">Endereço</div>'+
          '<div style="font-family:\'Plus Jakarta Sans\',sans-serif;font-size:32px;font-weight:800;color:var(--t)">'+end+'</div>'+
    ...
    '<div style="background:#fff;border-radius:14px;border:1px solid var(--gray2);padding:20px;box-shadow:var(--sh)">'+
      '<div style="font-family:\'Plus Jakarta Sans\',sans-serif;font-size:14px;font-weight:700;margin-bottom:12px">Minhas bipagens — Endereço '+end+(modo==='auditoria'?' (Rodada '+rodada+')':'')+'</div>'+
  ...
  var html=
    '<div id="modal-finalizar" onclick="if(event.target===this)fecharModalFinalizar()" '+
      ...
      '<div style="background:#fff;border-radius:18px 18px 0 0;padding:24px 20px 28px;width:100%;max-width:520px;max-height:82vh;display:flex;flex-direction:column;box-sizing:border-box">'+
        '<div style="font-family:\'Plus Jakarta Sans\',sans-serif;font-size:17px;font-weight:800;margin-bottom:2px">Finalizar Endereço</div>'+
  ...
  db.collection('inv_inventarios').doc(_invAtivo.id).get().then(function(snap){
    ...
    wrap.innerHTML=
      '<div style="display:flex;align-items:center;gap:16px;margin-bottom:16px;padding:16px;background:var(--gray);border-radius:12px;flex-wrap:wrap">'+
        '<div>'+
          '<div style="font-size:36px;font-weight:800;font-family:\'Plus Jakarta Sans\',sans-serif;line-height:1;color:'+corN+'">'+n+'</div>'+
...
document.addEventListener('keydown', function(ev){ if(_decisaoAberta()){ if(ev.target&&ev.target.id==='dup-qty-input') return; ev.preventDefault(); ev.stopImmediatePropagation(); } }, true);
...
// ── Código já coletado (qualquer endereço) → aviso + somar ─────────────────
...
function _checarJaColetado(invId, lido, res, semTeto) {
  ...
  var consulta=q.get().then(function(snap){
    var total=0, locais={};
    snap.docs.forEach(function(d){ var b=d.data(); var qn=Number(b.qty)||0; total+=qn; var k=(b.endereco||'?')+'|'+(b.coletorNome||b.coletorId||'?'); locais[k]=(locais[k]||0)+qn; });
...
function _abrirModalJaColetado(lido,res,qtyTotal,fator,prev){
  ...
  var html='<div id="modal-dup" style="position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:2300;display:flex;align-items:flex-end;justify-content:center">'+
    '<div style="background:#fff;border-radius:20px 20px 0 0;padding:20px;width:100%;max-width:480px">'+
    '<div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.6px;color:#b38600;margin-bottom:4px">Código já coletado neste inventário</div>'+
    '<div style="font-size:15px;font-weight:700;margin-bottom:6px">'+titulo+'</div>'+
    ...
    '</div></div></div>';
  document.body.insertAdjacentHTML('beforeend',html);
  setTimeout(function(){ var i=document.getElementById('dup-qty-input'); if(i){ i.focus(); i.select(); } },80);
}
function _dupQtyKeydown(ev){
  if (ev.key==='Enter'||ev.keyCode===13){ ev.preventDefault(); if(Date.now()-_dupAbertoTs<400) return false; _somarJaColetado(); return false; }
  return true;
}
...
function renderColetaAvulsa() {
  ...
  wrap.innerHTML=coletorChip+
    ...
    '</div>'+
    '<div class="card">'+
      '<div style="font-family:\'Plus Jakarta Sans\',sans-serif;font-size:14px;font-weight:700;margin-bottom:12px;display:flex;align-items:center;justify-content:space-between">'+
        'Bipagens Avulsas'+
...
function _alertar100pct(){
  ...
  t.style.cssText='position:fixed;top:20px;left:50%;transform:translateX(-50%);z-index:9999;background:#1a5c34;color:#fff;padding:16px 28px;border-radius:16px;font-weight:800;font-size:16px;font-family:\'Plus Jakarta Sans\',sans-serif;box-shadow:0 4px 24px rgba(0,0,0,.25);text-align:center;animation:none';
...
function _abrirSemEAN(){
  ...
  var html=
    '<div id="modal-sem-ean" onclick="if(event.target===this)_fecharSemEAN()" style="position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:2200;display:flex;align-items:flex-end;justify-content:center;padding:0">'+
      '<div style="background:#fff;border-radius:20px 20px 0 0;padding:24px 20px 36px;width:100%;max-width:480px;box-shadow:0 -4px 32px rgba(0,0,0,.18)">'+
        '<div style="font-family:\'Plus Jakarta Sans\',sans-serif;font-size:17px;font-weight:800;margin-bottom:4px">📝 Produto Sem Código</div>'+
